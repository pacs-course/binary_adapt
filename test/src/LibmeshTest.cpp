...
TEST_F (LibmeshTest, GeneralIntegration)
{
	...
	libMesh::MeshTools::Generation::build_line (mesh1, n, 1., 2.,
												LibmeshIntervalType);

	auto iter = mesh1.elements_begin();
	auto& element = *iter;
	LibmeshBinary::LibmeshIntervalClass* libmesh_interval =
		dynamic_cast<LibmeshBinary::LibmeshIntervalClass*> (element);
	ASSERT_NE (libmesh_interval, nullptr) << "LibmeshIntervalType not recognized "
										  << "casting to LibmeshBinary::LibmeshIntervalClass*";

	clog << "Constructing FElement" << endl;
	LibmeshBinary::FElement<1, LegendreType, LibmeshBinary::LibmeshIntervalClass>
	interval (iter);
	...
	double val = interval.Integrate ( [&order]						//*NOPAD*
	...
	clog << "Building a 2D mesh with "
		 << 2 * nx* ny
		 << " triangular elements in (1,2)x(1,2)"
		 << endl;

	libMesh::Mesh mesh2 (_mesh_init_ptr->comm());
	libMesh::MeshTools::Generation::build_square (mesh2, nx, ny, 1., 2., 1., 2.,
												  LibmeshTriangleType);
	...
	LibmeshBinary::LibmeshTriangleClass* libmesh_triangle =
		dynamic_cast<LibmeshBinary::LibmeshTriangleClass*> (element2);
	ASSERT_NE (libmesh_triangle, nullptr) << "LibmeshTriangleType not recognized "
										  << "casting to LibmeshBinary::LibmeshTriangleClass*";

	clog << "Constructing FElement" << endl;
	LibmeshBinary::FElement<2, WarpedType, LibmeshBinary::LibmeshTriangleClass>
	triangle (iter2);
	...
	clog << "Computing integral of x^" << order
		  << " + y^" << order
		  << " in  triangle co{(1,1),(2,1),(2,2)}"
		  << endl;

	val = triangle.Integrate ( [&order]												//*NOPAD*
...
TEST_F (LibmeshTest, BinaryElementsConstruction)
{
	...
	libMesh::MeshTools::Generation::build_line (mesh, n, 0., 1.,
												LibmeshIntervalType);
	...
	ASSERT_TRUE (LibmeshBinary::BinarityMap::CheckBinarity (
					 mesh)) << "After MakeBinary mesh is not binary!";
	...
	libMesh::MeshTools::Generation::build_square (mesh2, nx, ny, 0., 1., 0., 1.,
												  LibmeshTriangleType);

	auto& f_factory2 (BinaryTree::FunctionsFactory<2>::Instance());
	unique_ptr<BinaryTree::Functor<2>> smart_ptr2 =
		f_factory2.create ("x_squared_plus_y_squared");
	...
	ASSERT_TRUE (LibmeshBinary::BinarityMap::CheckBinarity (
					 mesh2)) << "After MakeBinary mesh is not binary!";
...
TEST_F (LibmeshTest, IntervalProjectionTest)
{
	...
	LibmeshBinary::BinarityMap::MakeBinary<1> (mesh_refiner, f_ptr);
	ASSERT_TRUE (LibmeshBinary::BinarityMap::CheckBinarity (mesh))
		<< "After MakeBinary mesh is not binary!";

	auto el = dynamic_cast<LibmeshBinary::BinaryTreeElement
			  <1,LegendreType, LibmeshBinary::LibmeshIntervalClass>*>
			  (mesh.elem_ptr (0));

	ASSERT_NE (el, nullptr) << "Mesh element not recognized "
							<< "casting to LibmeshBinary::BinaryElement";
	...
	EXPECT_EQ (el->PLevel(), 0) << "p level not modified yet, "
								<< "expected zero but it values "
								<< el->PLevel();
	...
	{
		el->PLevel (p);
		EXPECT_EQ (el->PLevel(), p) << "Element p level: expected "
									<< p << " is " << el->PLevel();

		double error = el->ProjectionError();
		clog << "Interpolation error on P"
			 << el->PLevel() << " : " << error << endl;
	}
	...
	for (size_t p = p_exact; p <= order - 2; ++p)
	{
		...
		EXPECT_LT (error, 1E-10) << "x^2 wrongly projected on P"
								 << p
...
TEST_F (LibmeshTest, SquareProjectionTest)
{
	...
	clog << "Building a 2D mesh with "
		 << nx* ny
		 << " squared elements in (0,1)x(0,1)"
		 << endl;

	libMesh::MeshTools::Generation::build_square (mesh, nx, ny, 0., 1., 0., 1.,
												  LibmeshSquareType);

	auto& f_factory (BinaryTree::FunctionsFactory<2>::Instance());
	unique_ptr<BinaryTree::Functor<2>> smart_ptr =
		f_factory.create ("x_squared_plus_y_squared");

	auto libmesh_ptr = mesh.elem_ptr (0);

	auto el = dynamic_cast<LibmeshBinary::LibmeshSquareClass*> (libmesh_ptr);
	ASSERT_NE (el, nullptr) << "Mesh element not recognized "
							<< "casting to LibmeshBinary::LibmeshSquareClass*";

	unique_ptr<LibmeshBinary::LibmeshSquareClass> smart_el (el);
	LibmeshBinary::FElement<2, LegendreType, LibmeshBinary::LibmeshSquareClass>
	f_el (move (smart_el));
...
TEST_F (LibmeshTest, TriangleOrthogonality)
{
	...
	clog << "Checking the basis orthogonality "
		 << "on the triangle co{(0,0),(1,0),(0,1)}:"
		 << endl;
...
TEST_F (LibmeshTest, TriangleProjectionTest)
{
	...
	auto el = dynamic_cast <LibmeshBinary::BinaryTreeElement
							<2, WarpedType, LibmeshBinary::LibmeshTriangleClass>*>
						  	(* (mesh.elements_begin()));
	ASSERT_NE (el, nullptr) << "Mesh element not recognized "
							<< "casting to LibmeshBinary::BinaryElement";
...
TEST_F (LibmeshTest, LibmeshRefinement)
{
	...
	libMesh::MeshTools::Generation::build_line (mesh, n, 1., 2.,
												LibmeshIntervalType);
	auto& element_ptr = * (mesh.elements_begin());

	ASSERT_NE (element_ptr, nullptr) << "Mesh element pointer is null";

	LibmeshBinary::LibmeshIntervalClass* copy_element_ptr =
		dynamic_cast<LibmeshBinary::LibmeshIntervalClass*> (element_ptr);
	ASSERT_NE (copy_element_ptr, nullptr) << "Element type not recognized "
										  << "casting to LibmeshBinary::LibmeshIntervalClass*";
	...
	ASSERT_TRUE (copy_element_ptr->operator== (*element_ptr))
	...
	EXPECT_TRUE (copy_element_ptr->operator== (** (mesh.elements_begin())) )
		<< "After the insert the element is different from the starting one";
...
TEST_F (LibmeshTest, TriangleBisection)
{
	...
	EXPECT_EQ (el->n_children(), 2) << "The expected triangular binary element has "
									<< el->n_children()
...
TEST_F (LibmeshTest, ManualBinaryRefinement)
{
	...
	LibmeshBinary::Interval* I =
		dynamic_cast<LibmeshBinary::Interval*> (mesh.elem (0));
	...
	EXPECT_NE (I->Left(), nullptr)
		<< "After refinement the interval has not left child";
	EXPECT_NE (I->Right(), nullptr)
		<< "After refinement the interval has not right child";

	auto hansel = dynamic_cast<LibmeshBinary::Interval*> (I->Left());
	auto gretel = dynamic_cast<LibmeshBinary::Interval*> (I->Right());
	ASSERT_NE (hansel, nullptr)
		<< "Binary element not recognized casting to LibmeshBinary::Interval*";
	ASSERT_NE (gretel, nullptr)
		<< "Binary element not recognized casting to LibmeshBinary::Interval*";
	...
	for (auto iter = mesh.active_elements_begin();
		iter != mesh.active_elements_end(); ++iter)
	++cont;
...
TEST_F (LibmeshTest, BinaryRefinement)
{
	...
	int n = 2;
	libMesh::MeshTools::Generation::build_line (*mesh_ptr, n, 0, 1,
												LibmeshIntervalType);
	...
	size_t n_iter (10);
	clog << "Starting Binary Tree hp Refinement with "
		 << n_iter
		 << " degrees of freedom"
		 << endl;
	...
	for (auto iter = mesh_ptr->elements_begin();
		 	  iter != mesh_ptr->elements_end();
			  ++iter)
		if ((*iter)->refinement_flag() == libMesh::Elem::INACTIVE)
			--cont;
		else
			cout << "Active Element of polinomial degree : "
				 << (*iter)->p_level()
				 << endl;

	cout << "I have started from a mesh with "
		 << n
		 << " elements; after refinement it has "
		 << mesh_ptr->n_elem()
		 << " elements, "
		 << cont
		 << " of them are ACTIVE"
	...
	cout << "The mesh has "
		 << binary_refiner.ActiveNodesNumber()
		 << " ACTIVE elements"
		 << endl;
...
TEST_F (LibmeshTest, IOTest)
{
	...
	int nx = 1, ny = 1;
	libMesh::MeshTools::Generation::build_square (mesh, nx, ny, 1., 2., 1., 2.,
												  LibmeshTriangleType);
...
TEST_F (LibmeshTest, FirstNotExactElement)
{
	...
	libMesh::Mesh mesh1 (_mesh_init_ptr->comm());
	libMesh::MeshTools::Generation::build_line (mesh1, n, 0., 1.,
												LibmeshIntervalType);
	...
	clog << "Let's check the normality of the element #" << i << " of the basis :"
		 << endl;

	double I = f_el.L2Norm ( [&]												// *NOPAD*
							 (const Point<1>& p)								// *NOPAD*
							 { return f_el.EvaluateBasisFunction (i, p);} );	// *NOPAD*
	I *= I * (i + 0.5);
