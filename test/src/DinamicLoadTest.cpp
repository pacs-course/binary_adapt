...
TEST_F(LoadTest, StdIntegration)
{
	...
	clog << "I compute the integral in (-1,1) of x^" << order << ", the highest power exactly integrated" << endl;
	double val = std_interval.Integrate([&order](const Point<1>& x){return pow(x[0], order);});
	...
	val = std_interval.Integrate([&order](const Point<1>& x){return pow(x[0], order);});
	solex = order % 2 ? 0 : 2;
	solex /= (order + 1);
	err = abs(val - solex);
...
//	clog << endl;
	...
	clog << "I compute the integral in (-1,1)x(-1,1) of x^" << order << " + y^" << order << endl;
	val = std_square.Integrate([&order](const Point<2>& x){return pow(x[0], order) + pow(x[1], order);});
	solex = order % 2 ? 0 : 2;
	solex /= (order + 1);
	solex *= 4;
	err = abs(val - solex);
	...
	clog << "I compute the integral on the standard triangle = co{(-1,-1),(1,-1),(-1,1)} of x^" << order << " + y^" << order << endl;
	val = std_square.Integrate([&order](const Point<2>& x){return pow(x[0], order) + pow(x[1], order);});
...
TEST_F(LoadTest, LegendreOrthonormality)
{
	...
	FiniteElements::StdFIperCube<1, FiniteElements::LegendreType> sfic;
	...
	clog << endl << endl << "Checking the normality on the standard interval :" << endl;
	...
	clog << endl << endl << "Checking the normality on the standard square :" << endl;
