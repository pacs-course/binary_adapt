	...
	**/
	class BinarityMap
	{
	  public:
		...
		**/
		template <size_t dim>
		static void MakeBinary (libMesh::MeshRefinement&,
		...
		**/
		template <size_t dim>
		static const BinaryTree::DimensionedNode<dim>*
		AsBinary (const libMesh::Elem*);
	...
	template <size_t dim>
	void BinarityMap::MakeBinary (libMesh::MeshRefinement& mesh_refinement,
								  BinaryTree::FunctionPtr<dim> f_ptr)
	{
		...
		*/
		...
		for (auto iter (mesh.elements_begin()); iter != mesh.elements_end(); ++iter)
		{
			...
			auto bin_ptr = BinarityMap::BinarizeNode<dim> (	el_ptr,
															f_ptr,
															mesh_refinement);
			/*
			...
			*/
			if (el_ptr != bin_ptr)
			{
				...
				/*
					...
					TODO: the first neighbor should be the father;
						  check if previous replacement could be avoided
				*/
		...
		libMesh::MeshRefinement& mesh_refinement)
	{
		...
		*/
		...
		/*
			I check if the element has been already casted
			(i.e. in adaptation,
			we want to transform only the children but not the father);
			WARNING : if I don't add this two lines
			the functions generates a segmentation fault error
			when calling the map initialization inside the el->init() instruction;
			the first line of AffineMap::init() cannot access the nodes_ptr address memory
		*/
		...
		if (!result)
		{
			...
			if (smart_ptr)
			{
				...
				/*  WARNING: when I go out from the for cicle libmesh detects
							 a memory leak due to the destruction of casted_ptr
							 without deleting the underlying object;
							 the problem is that this object is also pointed by *iter,
							 since it is an element of the mesh,
							 so it cannot be destroyed by calling delete casted_ptr */
			...
			if (!type_recognized)
				throw invalid_argument (
					"Cannot convert the input libMesh element into requested type!");
		}
	...
	libMesh::Elem* BinarityMap::BinarizeNode (
		...
		libMesh::MeshRefinement&)
	{
		/* In the default case (unexpected dim) it throws an exception;
		concrete behaviour for able-to-handle dim is defined in the specializations */
		throw invalid_argument (
