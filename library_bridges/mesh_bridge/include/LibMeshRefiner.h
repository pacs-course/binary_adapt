...
namespace LibmeshBinary
{
	...
	**/
	  ...
	  public :
		...
		**/
		...
		/**
			...
			Declared pure virtual in base class.
		**/
		virtual void IterateActiveNodes (BinaryTree::ConstOperator&)
		const override;
		...
		/**
			...
			Declared pure virtual in base class.
		**/
		virtual void IterateActiveNodes (BinaryTree::ConstDimOperator<dim>&)
		const override;
	...
	void LibmeshRefiner<dim>::SetMesh (std::shared_ptr<libMesh::MeshBase> mesh_ptr)
	{
		...
		this->_mesh_refinement_ptr =
			Helpers::MakeUnique<libMesh::MeshRefinement> (*(this->_mesh_ptr));
	...
	std::shared_ptr<libMesh::MeshBase> LibmeshRefiner<dim>::GetMesh() const
	{
		...
		if (this->_libmesh_init_ptr)
			throw logic_error (
				"The mesh can't be exported outside \
	...
	void LibmeshRefiner<dim>::ExportMesh (std::string output) const
	{
		...
		/* TODO:evaluate the other way given by libMesh to write a mesh
		   TODO:after the refinement evaluate if it is the case to "unbinarize" the mesh
				...
				but for the _elemlinks attribute it cannot be guaranteed the same
	...
	::IterateActiveNodes (BinaryTree::ConstDimOperator<dim>& func) const
	{
		std::for_each ( this->_mesh_ptr->active_elements_begin(),
						this->_mesh_ptr->active_elements_end(),
						[&func]													//*NOPAD*
	...
	{
		this->_libmesh_init_ptr =
			Helpers::MakeUnique<libMesh::LibMeshInit> (argc, argv);

		_mesh_ptr = std::make_shared<libMesh::Mesh> (this->_libmesh_init_ptr->comm(),
