...
namespace SandiaQuadrature
{
	...
	**/
	template <size_t dim>
	Geometry::QuadPointVec<dim>
	TensorizePoints (const Geometry::QuadPointVec<1>&);
	...
	**/
	template <size_t dim>
	Geometry::QuadWeightVec
	TensorizeWeights (const Geometry::QuadWeightVec&);
	...
	**/
	...
	SandiaQuadratureRule<dim>::SandiaQuadratureRule()
	{
		...
		string conf_file = thisfile.substr (0, thisfile.find_last_of ('/'))
						 + "/sandia_quadrature.conf";
#ifdef DEBUG
		cerr << "SandiaQuadratureRule: opening configuration file "
			 << conf_file
			 << endl;
#endif //DEBUG
	...
	{
		Geometry::QuadPointVec < dim - 1 > temp =
			TensorizePoints < dim - 1 > (one_d_p);
	...
	template <>
	Geometry::QuadPointVec<1>
	TensorizePoints<1> (const Geometry::QuadPointVec<1>&);

	template <size_t dim>
	Geometry::QuadWeightVec TensorizeWeights (
		const Geometry::QuadWeightVec& one_d_w)
	{
		Geometry::QuadWeightVec temp =
			TensorizeWeights < dim - 1 > (one_d_w);
	...
	template <>
	Geometry::QuadWeightVec
	TensorizeWeights<1> (const Geometry::QuadWeightVec& one_d_w);
