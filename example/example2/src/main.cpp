...
int main (int argc, char** argv)
{
	...
	cout << "Starting the algorithm with " << max_iter <<
		 " iterations and tolerance of " << tol << endl;
...
// *INDENT-OFF*
	...
	size_t n_iter = 0;
	function<void()> gnuplot_printer =
		[&n_iter, digits, &binary_refiner_ptr, results_dir]
		()
		{
			stringstream ss;
	...
	size_t m_iter = 0;
	function<void()> projection_printer = 
		[x_max, x_min, results_dir, digits, &m_iter, &binary_refiner_ptr]
		()
		{
			stringstream ss;
