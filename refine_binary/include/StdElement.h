...
/*
	...
	In a more general setting, it's not a priori excluded the possibility to define my std ipercube,
	on which the quadrature nodes and weigths are constructed by tensorization of the 1d counterparts;
	this is similarly already done for basis functions.

	However, in this case the default StdIperCube is set as InvalidElement,
	so the respectively stdInterval, stdSquare, stdCube are defined through specialization on the dim parameter.
*/
namespace detail
{
/**
	Ipercube aliases.
	Specialization of template using is not allowed, so this struct pattern is needed.
**/
	template <size_t dim>
	struct alias
	{
