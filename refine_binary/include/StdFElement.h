...
namespace FiniteElements
{
	  ...
	  public:
		...
		**/
		virtual double EvaluateBasisFunction (size_t ind,
											  const Geometry::Point<dim>&) const;
	...
	**/
	template <size_t dim>
	class StdFIperCube<dim, LegendreType> :
		public AlmostStdFIperCube<dim, LegendreType>
	{
	  public:
		/**
			default constructor
	...
	**/
	template <size_t dim,
			  Geometry::ElementType Type = Geometry::InvalidElementType,
			  BasisType FeType = InvalidFeType>
	class StdFElement : public StdFElementInterface<dim, FeType>
	{
	  ...
	  public:
		...
		**/
		virtual double
		EvaluateBasisFunction (size_t ind,
		...
		**/
		virtual Geometry::Vector
		EvaluateBasis (size_t degree,
					   const Geometry::Point<dim>& point) const;
		...
		**/
		virtual Geometry::Point<dim>
		MapBackward (const Geometry::Point<dim>&)const;

		/**
			Get the _ipercube_map evaluation
		**/
		virtual Geometry::Point<dim>
		MapForward (const Geometry::Point<dim>&) const;
	...
	{
		_std_geometry =
			Helpers::Builders<Geometry::StdIperCube<dim>>::BuildSingleton ();
	...
	template <size_t dim, BasisType FeType>
	double AlmostStdFIperCube<dim, FeType>
	::EvaluateBasisFunction (size_t ind,
							 const Geometry::Point<dim>& point) const
	{
...
//*INDENT-ON*
	...
	{
		_std_geometry =
			Helpers::Builders<Geometry::StdElement<dim, Type>>::BuildSingleton ();

		_std_cube =
			Helpers::Builders<StdFIperCube<dim, FeType>>::BuildSingleton ();
