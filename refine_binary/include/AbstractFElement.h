	...
	**/
		...
		public AbstractSpaceInterface<dim>
	{
	  public:
		...
		**/
		virtual double EvaluateBasisFunction (size_t,
	...
	void AbstractFElement<dim, FeType>::Init()
	{
		if (! (this->_initialized))
		{
			...
			auto& felem_factory (GenericFactory::StdFElementFactory
								 <dim, FeType> ::Instance());
	...
	template <size_t dim, BasisType FeType>
	Geometry::QuadPointVec<dim> AbstractFElement<dim, FeType>
	::GetQuadPoints() const
	{
		CheckInitialization();
	...
	::GetQuadWeights() const
	{
		...
		/*
			I'm taking advantage from the fact that _map is affine,
			so it has an evaluateJacobian() method
	...
	template <size_t dim, BasisType FeType>
	double AbstractFElement<dim, FeType>
	...
	//TODO: optimize storing already evaluated points
	template <size_t dim, BasisType FeType>
	Geometry::Vector AbstractFElement<dim, FeType>
	::EvaluateBasis (const Geometry::Point<dim>& point) const
	{
		CheckInitialization();
